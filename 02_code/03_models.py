...
# Core features for the econometric Probit baseline
...
def run_expanding_window_cv():
    ...
    # Feature columns (exclude targets and raw USREC)
    ...
    # We will save out-of-sample predictions here
    ...
    for horizon in [3, 6, 12]:
        print(f"\n==========================================")
        print(f"TRAINING MODELS FOR HORIZON: {horizon}m")
        print(f"==========================================")
        ...
        for test_year in range(start_test_year, end_test_year + 1):
            ...
            print(f"Test Year {test_year} | Train range: {train_df.index[0].strftime('%Y-%m')} to {train_df.index[-1].strftime('%Y-%m')} | Test size: {len(test_df)}", flush=True)
            ...
            # --- 1. Probit Model (Econometric Baseline) ---
            ...
            try:
                # Add limit to iterations to avoid hanging
                probit_model = sm.Probit(y_train, X_train_probit_const).fit(disp=0, maxiter=35)
                prob_probit = probit_model.predict(X_test_probit_const)
            except Exception as e:
                # Fallback to sklearn Logistic Regression on core features
            ...
            # --- 2. Logistic Regression (ML Baseline) ---
            lr_model = LogisticRegression(C=0.1, penalty="l2", random_state=42)
            lr_model.fit(X_train_scaled, y_train)
            prob_lr = lr_model.predict_proba(X_test_scaled)[:, 1]
            
            # --- 3. Random Forest (Tree-based ML) ---
            rf_model = RandomForestClassifier(n_estimators=100, max_depth=5, min_samples_split=4, random_state=42, n_jobs=-1)
            ...
            # Collect test results for this year
        ...
        # Save prediction results for this horizon
        ...
        print(f"Saved horizon {horizon}m predictions to models/predictions_{horizon}m.csv", flush=True)
